...
@section('process_content')
    ...
    <form method="post" action="{{ route('contractor.orders.process.details.store') }}" id="form" data-parsley-validate>
        {{ csrf_field() }}
        <div class="row mb-80">
            <div class="col-md-12">
                            ...
                            <tr data-equipment-id="{{$item->id}}">
                                ...
                                <td>
                                    ...
                                    <input type="text" required class="form-control date-from"
                                           ...
                                           value="{{ old('equipments.'.$item->id.'.from',$item->pivot->from) }}"
                                    />
                                </td>
                                <td>
                                    <input type="text" required class="form-control date-to"
                                           ...
                                           value="{{ old('equipments.'.$item->id.'.to',$item->pivot->to) }}"
                                    />
                                </td>
                                <td>
                                    <input type="number" min="0" max="1000" class="form-control qty"
                                       ...
                                       @if($loop->index==0) data-parsley-qtyrequired=".qty" @endif
                                           ...
                                           value="{{ old('equipments.'.$item->id.'.qty',$item->pivot->qty) }}"
                                    />
                                </td>
                            </tr>
                        @endforeach
    ...
    <script type="text/javascript">
        ...
        function format ( id ) {
            ...
            var template= '<div>' +
                    ...
                    '<textarea class="input-control w-100" max-length="1000" name="equipments['+id+'][notes]" id="equipments_'+id+'_notes">'+((equipments_map[id]!=undefined && equipments_map[id]['notes']!=undefined)?equipments_map[id]['notes']:'')+'</textarea>'+
                '</div>';
            return template;
        ...
        $('#equipment-table tbody').on('click', 'td.details-control', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                equipments_map[tr.data('equipment-id')]['notes']=$('#equipments_'+tr.data('equipment-id')+'_notes').val();
                row.child.hide();
