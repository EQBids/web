...
@section('process_content')
    ...
    @else
        ...
        <form method="post" action="{{ route('contractor.orders.edit.suppliers.store',[$order->id]) }}" data-parsley-validate>
            {{ csrf_field() }}
            {{ method_field('put') }}
...
@push('footer_scripts')
    <script type="text/javascript">
        ...
        var old_suppliers = {!! json_encode(old('suppliers',isset($old_suppliers)?$old_suppliers->pluck('id'):[])) !!};
