...
<div class="{{ isset($ul_classes)?$ul_classes:'' }}">
    <ul class="nav navbar-nav flex-column">
        @foreach($current_group as $category)
            <li class="nav-item" >
                <a class="" href="{{ route('contractor.equipment.category',[$category['slug']]) }}">
                ...
                @if(isset($categories[$category['id']]))
                    @include('web.contractor.equipment.category.menu_list',['current_group'=>$categories[$category['id']],'ul_classes'=>'collapse'])
                @endif
            </li>
        @endforeach
    </ul>
