...
<div class="{{ isset($ul_classes)?$ul_classes:'' }}">
    <ul class="nav navbar-nav flex-column">
        @foreach($current_group as $category)
            <li class="nav-item" >
                <a class="" href="{{ route('admin.equipments.category',[$category['slug']]) }}">
                ...
                @if(isset($categories[$category['id']]))
                    @include('web.admin.equipments.category.menu_list',['current_group'=>$categories[$category['id']],'ul_classes'=>'collapse'])
                @endif
            </li>
        @endforeach
    </ul>
